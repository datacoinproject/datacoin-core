...
void GeneratePrimeTable()
{
    ...
    nSieveFilterPrimes = (unsigned int)  gArgs.GetArg("-sievefilterprimes", nDefaultSieveFilterPrimes);
    nSieveFilterPrimes = std::max(std::min(nSieveFilterPrimes, nMaxSieveFilterPrimes), nMinSieveFilterPrimes);
    nL1CacheSize = (unsigned int)  gArgs.GetArg("-l1cachesize", nDefaultL1CacheSize);
    ...
    LogPrintf("GeneratePrimeTable() : setting nSieveExtensions = %u, nSieveSize = %u, nSieveFilterPrimes = %u, nL1CacheSize = %u\n", nSieveExtensions, nSieveSize, nSieveFilterPrimes, nL1CacheSize);
    ...
    for (unsigned int n = 2; n < nPrimeTableLimit; n++)
        if (!vfComposite[n])
            vPrimes.push_back(n);
    LogPrintf("GeneratePrimeTable() : prime table [1, %u] generated with %u primes\n", nPrimeTableLimit, (unsigned int) vPrimes.size());
}
...
void ResetMinerStatistics()
{
    nTotalTests = 0;
    nTotalBlocksFound = 0;
    vTotalChainsFound = std::vector<uint64_t> (nMaxChainLength, 0);
}
...
void PrintCompactStatistics(volatile unsigned int vFoundChainCounter[nMaxChainLength])
{
    ...
    for (unsigned int i = 0; i < nMaxChainLength; i++)
    {
        if (vFoundChainCounter[i]) {
			unsigned int iout=vFoundChainCounter[i];
...
{
    for(unsigned int nPrime: vPrimes)
    {
        if (nPrime > p)
        {
            p = nPrime;
...
{
    return (Params().GetConsensus().nTargetMinLength << nFractionalBits);
}

unsigned int TargetGetInitial()
{
    return (Params().GetConsensus().nTargetInitialLength << nFractionalBits);
}
...
{
    if (TargetGetLength(nBits) > Params().GetConsensus().nTargetMinLength)
        nBits -= (1 << nFractionalBits);
}
...
bool TargetGetMint(unsigned int nBits, uint64_t& nMint)
{
    ...
    nMint = bnMint.getuint256().GetUint64(0);
    if (nMint > nMintLimit)
    {
        nMint = 0;
...
bool TargetGetNext(unsigned int nBits, int64_t nInterval, int64_t nTargetSpacing, int64_t nActualSpacing, unsigned int& nBitsNext)
{
    ...
    else if (nFractionalDifficultyNew == nFractionalDifficultyMin && TargetGetLength(nBitsNext) > Params().GetConsensus().nTargetMinLength)
    {
...
bool MineProbablePrimeChain(CBlock& block, mpz_class& mpzFixedMultiplier, bool& fNewBlock, unsigned int& nTests, unsigned int& nPrimesHit, mpz_class& mpzHash, CBlockIndex* pindexPrev, unsigned int vChainsFound[nMaxChainLength], CSieveOfEratosthenes& sieve, CPrimalityTestParams& testParams)
{
    ...
    int64_t nStart = 0; // microsecond timer
    ...
    while (nTests < nTestsAtOnce && pindexPrev == chainActive.Tip())
    {
