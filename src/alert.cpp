...
//TODO: DATACOIN. Убираем возможность слать привелигированные сообщения от разработчиков
...
bool CAlert::RelayTo(CNode* pnode) const
{
    ...
    if (pnode->setKnown.insert(GetHash()).second)
    {
            ...
            GetAdjustedTime() < nRelayUntil)
        {
			...
			g_connman->PushMessage(pnode, msgMaker.Make("alert", *this));
...

    {
        ...
        for (map<uint256, CAlert>::iterator mi = mapAlerts.begin(); mi != mapAlerts.end();)
        {
            ...
            if (Cancels(alert))
            {
                ...
                uiInterface.NotifyAlertChanged(); //TODO: Datacoin (*mi).first, CT_DELETED);
                mapAlerts.erase(mi++);
            }
        ...
        if(AppliesToMe())
        {
            uiInterface.NotifyAlertChanged(); //TODO: Datacoin GetHash(), CT_NEW);
