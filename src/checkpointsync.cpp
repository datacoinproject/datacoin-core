...
//TODO: DATACOIN. Убираем привилегию разработчикам выбирать ветвь блокчейна
const std::string CSyncCheckpoint::strMainPubKey = "";
// "0448cfab2a5fd56b05e74219dba527e8a4a811045d8328547fe75374ab50e8236488af1603df8d1611d91dca0a3d35844488d3552c25b0dd865d80ee6625b322b0";
const std::string CSyncCheckpoint::strTestPubKey = "";
// "045588ccb6e7fe6e45b68dc4581b20fc6a8a706996724552c6d4bec249ac228fe0ef305b4d3b8ba81888a03ce4aa03ddf2fc7105a3c36fac4d49aac33031f702bd";
...
std::map<uint256, CBlock*> mapOrphanBlocks; //TODO: Datacoin Убрать!!!
...
{
    return (gArgs.GetBoolArg("-checkpointenforce", false) || gArgs.IsArgSet("-checkpointkey")); // checkpoint master node is always enforced
}

bool AcceptPendingSyncCheckpoint()
{
    LOCK(cs_hashSyncCheckpoint);
    if (!hashPendingCheckpoint.IsNull() && mapBlockIndex.count(hashPendingCheckpoint))
    {
        ...
        if (IsSyncCheckpointEnforced() && !chainActive.Contains(pindexCheckpoint))
        {
            ...
            if (!ActivateBestChain(state, Params(), NULL)) // TODO: Datacoin pindexCheckpoint))
            {
        ...
        {
            g_connman->ForEachNode([](CNode* pnode) //DATACOIN g_connman->ForEachNode([&checkpointMessage, &g_connman](CNode* pnode)
                {
                    checkpointMessage.RelayTo(pnode, g_connman.get());
                });
...
bool CheckSyncCheckpoint(const uint256& hashBlock, const CBlockIndex* pindexPrev)
{
    ...
    if (!hashConnectedBlock.IsNull() && mapBlockIndex.count(hashConnectedBlock))
...
bool ResetSyncCheckpoint()
{
    ...
    if (mapBlockIndex.count(hash) && !chainActive.Contains(mapBlockIndex[hash]))
    {
        ...
        CValidationState state;
        if (!ActivateBestChain(state, Params(), NULL)) // TODO: Datacoin заставить выбрать лучшей цепочку содержащую mapBlockIndex[hash]))
        {
    ...
    if (!WriteSyncCheckpoint((mapBlockIndex.count(hash) && chainActive.Contains(mapBlockIndex[hash]))? hash : Params().GenesisBlock().GetHash()))
...
{
    LOCK(cs_hashSyncCheckpoint);
    if (pfrom && !hashPendingCheckpoint.IsNull()  && (!mapBlockIndex.count(hashPendingCheckpoint)) && (!mapOrphanBlocks.count(hashPendingCheckpoint)))
...
bool SetCheckpointPrivKey(std::string strPrivKey)
{
    ...
    checkpoint.hashCheckpoint = Params().GenesisBlock().GetHash();
...
bool SendSyncCheckpoint(uint256 hashCheckpoint)
{
    ...
    // Relay checkpoint
    {
        g_connman->ForEachNode([&checkpoint](CNode* pnode) //DATACOIN g_connman->ForEachNode([&checkpoint, &g_connman](CNode* pnode)
                {
                    checkpoint.RelayTo(pnode, g_connman.get());
                });
    }
    return true;
}
